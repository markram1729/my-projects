...
%token WHILE IF SWITCH RETURN PREPROC LE STRING PRINT FUNCTION DO ARRAY ELSE STRUCT STRUCT_VAR FOR GE EQ NE INC DEC AND OR
%left AND OR 
%left LE GE EQ NEQ CP OP
%left '<' '>'
%right '='
%right UMINUS
%left '+' '-'
...
min : ID {push($1);}
	{
		...
		{
			int currscope=stack[index1-1];
			int previous_scope=returnscope($1,currscope);
			if(currscope==previous_scope)
				printf("\nError : Redeclaration of %s : Line %d\n",$1,printline());
			else
			{
				insert_dup($1,"int",currscope);
				
				int sg=returnscope($1,stack[index1-1]);
				g_addr+=4;
			}
		}
		else
